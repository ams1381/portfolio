...
export default function AmirPortfolioScene({setReadyToLoad} : {setReadyToLoad : any}) {
    ...
    return (
        <>
            ...
            <Canvas camera={{ position: [-7, 3, -15], fov: 60 }} shadows={'soft'} className={'h-full '} style={{height: 700}} >
                ...
                <GarageModel scale={0.022} />
                <LightsComponent/>
                {/*{ start ? <LightsComponent/> : ''}*/}
                <CameraMover />
                {/*<Shelf position={[1, 2, 4]} />*/}
...
const LightsComponent = () => {
    ...
    const { ambientIntensity,castShadow, ambientColor } = useControls('abient light',{
        ambientIntensity: { value: 0.15, min: 0, max: 2 },
        castShadow : true ,
        ambientColor: '#ffffff'
    });
