...
export function UniversitySection({activeView} : {activeView : TAboutMeActiveView}) {
    ...
    const options = useControls('text options',{
        position : isMobile ? [-4,-58,-81] :  [23,-13,-4] ,
        rotation :[-1,3,4.8]
    })
    const utOptions = useControls('ut options',{
        position :!isMobile ? [0,0,0] : [-30,31,-66] ,
        rotation :!isMobile ? [0,0,0] : [0.6,-0.08,0] ,
    })
    return (
        <group
            ...
            <mesh scale={10} position={utOptions.position} rotation={utOptions.rotation}>
